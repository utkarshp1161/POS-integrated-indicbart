...
class GenerationMixin:
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        encoder_kwargs[model_input_name] = inputs_tensor  # encoder_kwargs["input_ids"] = inputs_tensor
        # print
    ...
    def _get_logits_processor(
        ...
        encoder_input_ids: torch.LongTensor, # inputs_tensor 
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        pad_token_id = pad_token_id if pad_token_id is not None else self.config.pad_token_id #0
        eos_token_id = eos_token_id if eos_token_id is not None else self.config.eos_token_id # 64001
        ...
        output_scores = output_scores if output_scores is not None else self.config.output_scores #false
        ...
        #self.model_kwargs_later = model_kwargs
        #self.before_prepare_model_inputs = inputs, model_kwargs #
        """(tensor([[    2,   954,   437,   278,  1157,    45, 24061,    65,   122, 64001, 64006,     3]]), {})"""
        inputs_tensor, model_input_name, model_kwargs = self._prepare_model_inputs(inputs, bos_token_id, model_kwargs)
        #self.after_prepare_model_inputs = inputs_tensor, model_input_name,  model_kwargs
        # inputs_tensor -- tensor([[    2,   954,   437,   278,  1157,    45, 24061,    65,   122, 64001, 64006,     3]])
        # model_input_name-- input_ids
        # model_kwargs - dict --dict_keys(['output_attentions', 'output_hidden_states', 'use_cache', 'attention_mask', 'encoder_outputs'])
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs: #false due to "encoder_outputs"
        ...
        #self.before_prepare_decoder_inputs = input_ids--not created
        if self.config.is_encoder_decoder:  #true
            input_ids = self._prepare_decoder_input_ids_for_generation( 
                ...
                device=inputs_tensor.device,  #cuda
            )
        else:
            # if decoder-only then inputs_tensor has to be `input_ids`
            input_ids = inputs_tensor
        
        self.after_prepare_decoder_inputs = input_ids #tensor([[1]])

        input_ids_seq_length = input_ids.shape[-1] #1
        ...
        #print
        #self.generation_mode = is_greedy_gen_mode, is_sample_gen_mode, is_beam_gen_mode, is_beam_sample_gen_mode, is_group_beam_gen_mode
        ...
        if is_greedy_gen_mode:
            ...
            #self.inputs_tensor_ = inputs_tensor #tensor([[    2,   954,   437,   278,  1157,    45, 24061,    65,   122, 64001, 64006,     3]])
            #self.before_expand_inputs = input_ids #tensor([[1]])
            input_ids, model_kwargs = self._expand_inputs_for_generation(
                input_ids, expand_size=num_beams, is_encoder_decoder=self.config.is_encoder_decoder, **model_kwargs
            )
            #self.after_expand_inputs = input_ids #tensor([[1],[1],[1],[1]]) # for beam search 4 
            # 12. run beam search
            self.logits_process = logits_processor
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            self.final_model_inputs = model_inputs

            outputs = self( #### print   change here
                **model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
